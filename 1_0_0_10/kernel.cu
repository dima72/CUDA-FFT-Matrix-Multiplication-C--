...
// FFT data
string g_program_version = "1.0.0.10";


string g_MatrixSuffix;

string g_U_matrix_fileName;
string g_E_matrix_fileName;
string g_D_matrix_fileName;
string g_W_matrix_fileName;
string g_WD_matrix_fileName;
string g_G_fileName;
string g_Indxs_fileName;
string g_UL_filename;
string g_P_matrix_at_step; //filename to save P matrix at step defined by g_PrintPropogateMatrixStepNum
string g_UL_matrix_at_step;//filename to save UL matrix at step defined by g_PrintPropogateMatrixStepNum
int g_PrintPropogateMatrixStepNum = -1; //Step number to save Propogate matrix. If value == -1 then not used at all.
...
unsigned int meshSizeX = 256;//4;//32 196;
unsigned int meshSizeY = 256;//4;//32 196;

unsigned int meshSize1D = meshSizeX * meshSizeY;
int g_MatrixMemSize = meshSizeX * meshSizeY * sizeof(Complex);
unsigned int ThreadsInBlock = 256;
unsigned int g_PropogateStepRange = 0;
...
void print2DArray(Complex* p_inMatrix, int p_Xsize, int p_Ysize);
...
//void Transform2Dto1D(Complex(&p_inMatrix)[meshSizeX][meshSizeY], Complex* p_outMatrix);
...
void write2DArrayToFile(Complex* p_inMatrix, string p_FileName);
...
void RefreshPropogateMatrix(int p_StepCount, Complex *p_inMatrix);
...
int main()
{
    try
    {
        ...
        CSimpleIniA ini;
        ini.SetUnicode();
        string a_iniFn = getCurrentDirectoryOnWindows() + "//cudafft.ini";
        SI_Error rc = ini.LoadFile(a_iniFn.c_str());
        if (rc < 0) { /* handle error */ };
        CheckError(rc == SI_OK, a_iniFn + "  ini.LoadFile failed");


        g_MatrixSuffix = ini.GetValue("settings", "MatrixSuffix", "8");
        CheckError(!g_MatrixSuffix.empty(), "variable MatrixSuffix is empty in ini");
        g_number_of_cycles = ini.GetLongValue("settings", "number_of_cycles", 10000);
        meshSizeX = ini.GetLongValue("settings", "meshSizeX", 256);
        meshSizeY = ini.GetLongValue("settings", "meshSizeY", 256);
        ThreadsInBlock = ini.GetLongValue("settings", "ThreadsInBlock", 256);
        g_PropogateStepRange = ini.GetLongValue("settings", "PropogateStepRange", 100);


        //g_print_forward_matrix = ini.GetBoolValue("settings", "print_forward_matrix", false);
        //g_print_forward_matrix_mul = ini.GetBoolValue("settings", "print_forward_matrix_mul", false);


        //ini.SetValue("section", "key", "newvalue");

        //pv = ini.GetValue("section", "key", "default");
        //CheckError(pv, "newvalue");


        meshSize1D = meshSizeX * meshSizeY;
        g_MatrixMemSize = meshSizeX * meshSizeY * sizeof(Complex);



        g_U_matrix_fileName = string("U") + g_MatrixSuffix +".txt";
        g_E_matrix_fileName = string("E") + g_MatrixSuffix +".txt";
        g_D_matrix_fileName = string("D") + g_MatrixSuffix +"_10000.txt";
        g_W_matrix_fileName = string("W") + g_MatrixSuffix +"_1cycle.txt";
        g_WD_matrix_fileName = string("WD") + g_MatrixSuffix +"_1cycle.txt";
        g_G_fileName = string("G") + g_MatrixSuffix +".txt";
        g_Indxs_fileName = string("Indxs")+ g_MatrixSuffix +".txt";
        g_UL_filename = string("UL") + g_MatrixSuffix + "_10000_E"+g_MatrixSuffix+".txt";
        g_P_matrix_at_step = string("P") + g_MatrixSuffix + "_first_step.txt"; //filename to save P matrix at step defined by g_PrintPropogateMatrixStepNum
        g_UL_matrix_at_step = string("UL") + g_MatrixSuffix + "_first_step.txt";//filename to save UL matrix at step defined by g_PrintPropogateMatrixStepNum
        ...
        printf("ThreadsInBlock %d\n", ThreadsInBlock);
        printf("PropogateStepRange %d\n\n", g_PropogateStepRange);
...
int RunStack2DTest()
{
    ...
    MemSet1DArray(h_P, g_MatrixMemSize, 0, 0);
    MemSet1DArray(h_U, g_MatrixMemSize, 0, 0);
    MemSet1DArray(h_Uout, g_MatrixMemSize, 0, 0);
    MemSet1DArray(h_Difract, g_MatrixMemSize, 0, 0);
    MemSet1DArray(h_E, g_MatrixMemSize, 0, 0);
    MemSet1DArray(h_G, g_MatrixMemSize, 0, 0);
...
//            LogMatrixMultiplication(h_W, h_Uout);
 //       }
        ...
        div_res = div(i, g_PropogateStepRange);
        if (div_res.rem == 0)
        {
...
//        MemSet1DArray(h_U_original,g_MatrixMemSize, 0, 0);
...
{    
    ...
    for (int i = 0; i < meshSizeX; i++)
    {
        for (int j = 0; j < meshSizeY;j++)
        {
            ...
            p_inMatrix[i + j * meshSizeX].x = h_G[G_I + G_J * meshSizeX].x;
            p_inMatrix[i + j * meshSizeX].y = h_G[G_I + G_J * meshSizeX].y;
...
void Initialization()
{
    ...
    h_U = new Complex[g_MatrixMemSize];
    h_Uout = new Complex[g_MatrixMemSize];
    h_Difract = new Complex[g_MatrixMemSize];
    h_E = new Complex[g_MatrixMemSize];
    h_P = new Complex[g_MatrixMemSize];
    h_W = new Complex[g_MatrixMemSize];
    h_G = new Complex[g_MatrixMemSize];
}
...
void print2DArray(Complex* p_inMatrix, int p_Xsize, int p_Ysize)
{
    for (int i = 0; i < p_Xsize; i++)
    {
        ...
        {
            Complex* aComp = reinterpret_cast<Complex*>(&p_inMatrix[i + j*meshSizeX]);
            printf("%f %f %d %d", aComp->x, aComp->y, i, j);
            printf("\n");
        }
    }
}
...
void read2DArrayFromFile(Complex *p_inMatrix, string p_FileName)
{
    ...
    while (!inFile.eof())//(getline(inFile, line))
    {
        ...
        {            
            p_inMatrix[I + J * meshSizeX].x = a_Comp.x;
            p_inMatrix[I + J * meshSizeX].y = a_Comp.y;
        }
    }
}
...
void write2DArrayToFile(Complex *p_inMatrix, string p_FileName)
{
    ...
    for (int i = 0; i < meshSizeX; i++)
    {
        for (int j = 0; j < meshSizeY; j++)
        {
            outFile << std::setw(17) << std::fixed << p_inMatrix[a_XYind].x;
            outFile << " ";
            outFile << std::setw(17) << std::fixed << p_inMatrix[a_XYind].y;
            outFile << " ";
            outFile << i;
            outFile << " ";
            outFile << j;

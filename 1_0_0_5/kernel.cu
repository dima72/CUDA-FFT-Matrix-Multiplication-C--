...
// FFT data
string g_program_version = "1.0.0.5";

const unsigned int meshSizeX = 1024;//4;//32 196;
const unsigned int meshSizeY = 1024;//4;//32 196;

string g_U_matrix_fileName = "U10.txt";
string g_D_matrix_fileName = "D10_1cycle.txt";
string g_W_matrix_fileName = "W10_1cycle.txt";
string g_WD_matrix_fileName = "WD10_1cycle.txt";
string g_UL_filename = "UL10_1cycle.txt";
...
Complex g_h_2dpropogate[meshSizeX][meshSizeY];
...
int main()
{
    try
    {
        ...
        printf("U matrix input fileName = %s\n", g_U_matrix_fileName.c_str());
        printf("D matrix input fileName = %s\n", g_D_matrix_fileName.c_str());
...
//    a_UseHostNormalization = (a_Choice == 1);
    ...
    for (int i = 0; i < g_number_of_cycles; i++)
    {
        ...
        if (a_ProduceIntermediateFiles)
        {
            ...
            write2DArrayToFile(h_Uout, "Output//" + g_W_matrix_fileName, 1);
        }
        ...
        if (a_ProduceIntermediateFiles)
        {
            ...
            print2DArray(h_Uout, 4, 4, 1);
            write2DArrayToFile(h_Uout, "Output//" + g_WD_matrix_fileName, 1);
        }
        ...
        {                                                        // result of division    
            divKernelComplex <<< 4, meshSizeX >>> (meshSize1D, d_U, d_Uinverse, meshSize1D);//final result is in d_U 
        }
        else
        {
            checkCudaErrors(cudaMemcpy(h_U, d_Uinverse, g_MatrixMemSize, cudaMemcpyDeviceToHost));
            NormalyzeFFT(h_U, meshSize1D);
            checkCudaErrors(cudaMemcpy(d_U, h_U, g_MatrixMemSize, cudaMemcpyHostToDevice));//final result is in d_U
        }
    }
